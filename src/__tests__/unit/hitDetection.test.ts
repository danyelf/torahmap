...
describe('hitDetection', () => {
  ...
  describe('isPointInVerse', () => {
    ...
    it('returns true when point is inside verse bounds', () => {
      expect(isPointInVerse(125, 225, verse)).toBe(true);
      expect(isPointInVerse(100, 200, verse)).toBe(true); // Top-left corner
      expect(isPointInVerse(149, 249, verse)).toBe(true); // Just inside
    });

    it('returns false when point is outside verse bounds', () => {
      expect(isPointInVerse(99, 225, verse)).toBe(false); // Left of verse
      expect(isPointInVerse(150, 199, verse)).toBe(false); // Above verse
      expect(isPointInVerse(125, 250, verse)).toBe(false); // Below verse (exclusive)
      expect(isPointInVerse(150, 225, verse)).toBe(false); // Right of verse (exclusive)
    });

    it('handles point at exact right edge (exclusive)', () => {
      // x + size is exclusive
      expect(isPointInVerse(150, 225, verse)).toBe(false);
    });

    it('handles point at exact bottom edge (exclusive)', () => {
      // y + size is exclusive
      expect(isPointInVerse(125, 250, verse)).toBe(false);
    });

    it('handles point at top-left corner (inclusive)', () => {
      expect(isPointInVerse(100, 200, verse)).toBe(true);
    });
    ...
    it('handles very small verse', () => {
      ...
      expect(isPointInVerse(0, 0, smallVerse)).toBe(true);
      expect(isPointInVerse(0.5, 0.5, smallVerse)).toBe(true);
      expect(isPointInVerse(1, 0.5, smallVerse)).toBe(false);
    });
  });
  ...
  describe('findVerseAtPoint', () => {
    ...
    it('finds verse using exact hit when available', () => {
      ...
      const hit = findVerseAtPoint(verses, camera, 5, 5);
    ...
    it('falls back to fuzzy hit when no exact match', () => {
      ...
      const hit = findVerseAtPoint(verses, camera, 12, 5);
    ...
    it('returns null when no exact or fuzzy hit', () => {
      ...
      const hit = findVerseAtPoint(verses, camera, 100, 100);
    ...
    it('handles camera zoom', () => {
      ...
      const hit = findVerseAtPoint(verses, camera, 10, 10);
    ...
    it('handles camera pan', () => {
      ...
      const hit = findVerseAtPoint(verses, camera, 105, 105);
    ...
    it('handles combined zoom and pan', () => {
      ...
      const hit = findVerseAtPoint(verses, camera, 60, 60);
    ...
    it('handles negative world coordinates after transformation', () => {
      ...
      const hit = findVerseAtPoint(verses, camera, 0, 0);
  ...
  describe('integration', () => {
    it('supports typical hover workflow with camera', () => {
      ...
      const hit4 = findVerseAtPoint(verses, camera, 210, 410);
      ...
      const hit5 = findVerseAtPoint(verses, camera, 310, 610);
    ...
    it('handles verse grid navigation', () => {
      ...
      expect(findVerseAtPoint(verses, camera, 5, 5)).toBe(verses[0]); // (0,0)
      expect(findVerseAtPoint(verses, camera, 25, 5)).toBe(verses[1]); // (1,0)
      expect(findVerseAtPoint(verses, camera, 45, 5)).toBe(verses[2]); // (2,0)
      expect(findVerseAtPoint(verses, camera, 5, 25)).toBe(verses[3]); // (0,1)
      expect(findVerseAtPoint(verses, camera, 25, 25)).toBe(verses[4]); // (1,1)
    });
  });
});
