...
describe('hitDetection', () => {
  ...
  describe('findItemAtPoint', () => {
    ...
    it('finds verse using exact hit when available', () => {
      ...
      const hit = findItemAtPoint(verses, camera, 5, 5);
    ...
    it('falls back to fuzzy hit when no exact match', () => {
      ...
      const hit = findItemAtPoint(verses, camera, 12, 5);
    ...
    it('returns null when no exact or fuzzy hit', () => {
      ...
      const hit = findItemAtPoint(verses, camera, 100, 100);
    ...
    it('handles camera zoom', () => {
      ...
      const hit = findItemAtPoint(verses, camera, 10, 10);
    ...
    it('handles camera pan', () => {
      ...
      const hit = findItemAtPoint(verses, camera, 105, 105);
    ...
    it('handles combined zoom and pan', () => {
      ...
      const hit = findItemAtPoint(verses, camera, 60, 60);
    ...
    it('handles negative world coordinates after transformation', () => {
      ...
      const hit = findItemAtPoint(verses, camera, 0, 0);
  ...
  describe('integration', () => {
    ...
    it('handles verse grid navigation', () => {
      ...
      expect(findItemAtPoint(verses, camera, 5, 5)).toBe(verses[0]); // (0,0)
      expect(findItemAtPoint(verses, camera, 25, 5)).toBe(verses[1]); // (1,0)
      expect(findItemAtPoint(verses, camera, 45, 5)).toBe(verses[2]); // (2,0)
      expect(findItemAtPoint(verses, camera, 5, 25)).toBe(verses[3]); // (0,1)
      expect(findItemAtPoint(verses, camera, 25, 25)).toBe(verses[4]); // (1,1)
    });
  });
});
