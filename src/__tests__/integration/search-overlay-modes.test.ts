...
describe('Search Overlay - Hebrew Mode Integration', () => {
  ...
  describe('Mode Switching Behavior', () => {
    it('switching mode by restoring a link runs the new search', () => {
      ...
      const exodus = testVerses.find((v) => v.book === 'Exodus' && v.chapter === 3)!;
      expect(searchOverlay.getVerseColor(testVerses[1])).toEqual(SEARCH_COLORS[0]);
      expect(searchOverlay.getVerseColor(exodus)).not.toEqual(SEARCH_COLORS[0]);
  ...
  describe('URL State Persistence', () => {
    it('the link includes Hebrew mode when set', () => {
      searchOverlay.renderControls?.(container);
    ...
    it('restoring a link restores Hebrew word mode', () => {
      ...
      // Verify mode was set in the link
      const params = searchOverlay.toUrl();
      expect(params!.mode).toBe('w');
    ...
    it('restoring a link leaves a term on its default for an unknown entry', () => {
      ...
      expect(searchOverlay.toUrl().mode).toBeUndefined();
      expect(markedMode()).toBe('meanings');
    });
    ...
    it('round-trips URL state correctly', () => {
      ...
      // Verify mode persisted in the link
      const restoredParams = searchOverlay.toUrl();
  ...
  describe('Mode Persistence Across Re-renders', () => {
    it('preserves Hebrew mode when re-rendering controls', () => {
      ...
      // Mode should be preserved in the link
      const newParams = searchOverlay.toUrl();
