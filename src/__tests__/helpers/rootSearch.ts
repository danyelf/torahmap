// Root-mode search, run the way the search overlay runs it.
//
// The overlay stopped asking search() for root mode when a term gained a choice
...
// the reader gets before touching anything. Tests can then describe root-mode
...
export function searchInRootMode(query: string): SearchResult[] {
  const terms = query
    .split(',')
    .map((part) => part.trim())
  ...
  return resultsForVerseSets(
    terms.map((term) =>
      ...
      // else falls back to whole-word matching, as root mode always has.
