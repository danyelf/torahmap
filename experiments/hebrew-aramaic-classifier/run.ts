...
const FIXTURES: Fixture[] = [
  {
    ...
    text: 'אִילֵּימָא לְמִבְטַל טוּמְאָתַהּ, תְּנֵינָא: כׇּל הַכֵּלִים יוֹרְדִין לִידֵי טוּמְאָתָן בְּמַחְשָׁבָה, וְאֵין עוֹלִין מִטּוּמְאָתָן אֶלָּא בְּשִׁינּוּי מַעֲשֶׂה.',
    //  אילימא למבטל טומאתה  תנינא  כל  הכלים יורדין לידי טומאתן במחשבה ואין עולין מטומאתן אלא  בשינוי מעשה
    expected: ['A', 'A', 'A', 'A', 'N', 'H', 'H', 'N', 'H', 'H', 'H', 'H', 'H', 'H', 'H', 'H'],
  },
  {
    name: 'three-switch (gelalim, includes lo/la minimal pair)',
    text: 'כִּכְלֵי גְלָלִים, כִּכְלֵי אֲדָמָה, וְאֵין מְקַבְּלִין טוּמְאָה – דְּאָמַר מָר: כְּלֵי אֲבָנִים וּכְלֵי גְלָלִים וּכְלֵי אֲדָמָה אֵין מְקַבְּלִין טוּמְאָה לֹא מִדִּבְרֵי תוֹרָה וְלֹא מִדִּבְרֵי סוֹפְרִים, אוֹ דִלְמָא לָא הָוֵי עִיכּוּל?',
...
function colorLabel(label: Label): string {
  switch (label) {
    case 'A':
      return COLOR.cyan + 'A' + COLOR.reset;
    case 'H':
      return COLOR.yellow + 'H' + COLOR.reset;
    case 'N':
      return COLOR.dim + 'N' + COLOR.reset;
    case '?':
...
function runFixtures(classifier: LanguageClassifier): void {
  ...
  for (const fixture of FIXTURES) {
    ...
    if (tokens.length !== fixture.expected.length) {
      ...
      console.log(`Tokens: ${tokens.map((t) => t.stripped).join(' / ')}`);
      console.log();
      continue;
    }
...
async function runBerakhot(classifier: LanguageClassifier): Promise<void> {
  ...
  console.log(`Loaded ${segments.length} segments across ${data.text.length} amudim.\n`);

  // Stage 3: sample some segments and print labels
  console.log(
    `${COLOR.blue}=== Stage 3: sample segments (first segment of dapim 2a, 5a, 17b, 31a, 54a) ===${COLOR.reset}\n`,
  );
  ...
  for (const targetDaf of sampleDapim) {
    ...
    console.log(
      `  Labels:  ${packLabels(result.labels)
        .split('')
        .map((l) => colorLabel(l as Label))
        .join('')}`,
    );
    console.log(
  ...
  console.log(
    `    Aramaic    A: ${totalA.toString().padStart(6)}  ${fmtPct(totalA / totalWords)} of total, ${fmtPct(totalA / voting)} of voting`,
  );
  console.log(
    `    Hebrew     H: ${totalH.toString().padStart(6)}  ${fmtPct(totalH / totalWords)} of total, ${fmtPct(totalH / voting)} of voting`,
  );
  console.log(
    `    Neutral    N: ${totalN.toString().padStart(6)}  ${fmtPct(totalN / totalWords)} of total`,
  );
  console.log(
    `    Unknown    ?: ${totalUnknown.toString().padStart(6)}  ${fmtPct(totalUnknown / totalWords)} of total`,
  );
  console.log();
  console.log(`  Segment-level dominance (≥70% threshold):`);
  console.log(
    `    Aramaic-dominant:  ${dominant.A.toString().padStart(5)}  ${fmtPct(dominant.A / segmentsClassified)}`,
  );
  console.log(
    `    Hebrew-dominant:   ${dominant.H.toString().padStart(5)}  ${fmtPct(dominant.H / segmentsClassified)}`,
  );
  console.log(
    `    Mixed:             ${dominant.mixed.toString().padStart(5)}  ${fmtPct(dominant.mixed / segmentsClassified)}`,
  );
  console.log(
    `    All neutral/unk:   ${dominant.neutral.toString().padStart(5)}  ${fmtPct(dominant.neutral / segmentsClassified)}`,
  );
  console.log();
}
